...
public class CooeeClient {
    ...
    public static WeatherEntity getWeatherInfo(Context context,
            String city_num, int what) {
        ...
        if (NumberClockHelper.isHaveInternet(context)) {
            ...
            try {
                ...
                if (status == 200) {
                    ...
                    responseReader = new InputStreamReader(entity.getContent(),
                            "GB2312");
                    dataentity = CooeeWeatherParseWeatherData(context,
                            responseReader, timeUTC, what);
                    // what为1设置城市里面的搜索城市，what为2曲线里面的更新城市，what为3widget的刷新城市，widget为4自动更新的刷新城市
                    if (what == 1) {
                        if (dataentity != null
                                && dataentity.getDetails() != null
                                && dataentity.getDetails().size() == 5) {
                            Bundle bundle = new Bundle();
                            bundle.putSerializable(
                                    Parameter.SerializableCitySearchName,
                                    dataentity);
                            if (CityFinderView.mHandler != null) {
                                CityFinderView.mHandler.obtainMessage(
                                        CityFinderView.MSG_SUCCESS, bundle)
                                        .sendToTarget();
                            }
                        } else {
                            if (CityFinderView.mHandler != null) {
                                CityFinderView.mHandler.obtainMessage(
                                        CityFinderView.MSG_FAILURE)
                                        .sendToTarget();
                            }
                        }
                    } else if (what == 2) {
                        if (dataentity != null
                            ...
                            Bundle bundle = new Bundle();
                            bundle.putSerializable(
                                    Parameter.SerializableCurveFlushName,
                                    dataentity);
                            if (WeatherCurveView.mHandler != null) {
                                WeatherCurveView.mHandler.obtainMessage(
                                        WeatherCurveView.MSG_SUCCESS, bundle)
                                        .sendToTarget();
                            }
                        } else {
                            if (WeatherCurveView.mHandler != null) {
                                WeatherCurveView.mHandler.obtainMessage(
                                        WeatherCurveView.MSG_FAILURE)
                                        .sendToTarget();
                            }
                        }
                    } else if (what == 4) {
                        if (dataentity != null
                            ...
                            Bundle bundle = new Bundle();
                            bundle.putSerializable(
                                    Parameter.SerializableAutoUpdateName,
                                    dataentity);
                            if (UpDateWeatherHelper.mHandler != null) {
                                UpDateWeatherHelper.mHandler
                                        .obtainMessage(
                                                UpDateWeatherHelper.MSG_AUTOUPDATE_INLANDSUCCESS,
                                                bundle).sendToTarget();
                            }
                        } else {
                            if (UpDateWeatherHelper.mHandler != null) {
                                UpDateWeatherHelper.mHandler
                                        .obtainMessage(
                                                UpDateWeatherHelper.MSG_AUTOUPDATE_INLANDFAILURE)
                                        .sendToTarget();
                            }
                        }
                    } else {
                ...
                if (what == 1) {
                    if (CityFinderView.mHandler != null) {
                        CityFinderView.mHandler.obtainMessage(
                                CityFinderView.MSG_FAILURE).sendToTarget();
                    }
                } else if (what == 2) {
                    if (WeatherCurveView.mHandler != null) {
                        WeatherCurveView.mHandler.obtainMessage(
                                WeatherCurveView.MSG_FAILURE).sendToTarget();
                    }
            ...
            if (what == 1) {
                if (CityFinderView.mHandler != null) {
                    CityFinderView.mHandler.obtainMessage(
                            CityFinderView.MSG_NETWORK_FAILURE).sendToTarget();
                }

            } else if (what == 2) {
                if (WeatherCurveView.mHandler != null) {
                    WeatherCurveView.mHandler.obtainMessage(
                            WeatherCurveView.MSG_NETWORK_FAILURE)
                            .sendToTarget();
                }

            } else if (what == 4) {
                if (UpDateWeatherHelper.mHandler != null) {
                    UpDateWeatherHelper.mHandler.obtainMessage(
                            UpDateWeatherHelper.MSG_AUTOUPDATE_NETWORK_FAILURE)
                            .sendToTarget();
                }
    ...
    public static WeatherEntity CooeeWeatherParseWeatherData(Context context,
            Reader responseReader, long time, int what) {
    ...
    public static void queryCurWeatherData(String s, WeatherEntity dataEntity,
            long time) {
        ...
        dataEntity.setTempC(CalTempC(time,
                Integer.parseInt(dataEntity.getTempH()),
                Integer.parseInt(dataEntity.getTempL())));
    }

    private static void getLunarCalendar(String s, WeatherEntity dataEntity) {
        String lunar1[] = s.split("<nl>");
        String lunar2[] = lunar1[1].split("</nl>");
        dataEntity.setLunarcalendar(lunar2[0]);
    }

    private static void getHumidity(String s, WeatherEntity dataEntity) {
        String humidity1[] = s.split("<sd>");
        String humidity2[] = humidity1[1].split("</sd>");
        dataEntity.setHumidity(humidity2[0]);
    }

    private static void getWindData(String s, WeatherEntity dataEntity) {
    ...
    public static void getCondition(String s, WeatherEntity dataEntity) {
        String t[] = s.split("\\,");
        if (t[0].equals(s)) {
    ...
    public static void CooeeCurFormatTemprature(WeatherEntity dataEntity,
            String temp) {
        String[] s = temp.split("\\,");
        String high = s[1];
        String low = s[0];
    ...
    private static Integer CalTempC(final long ld, Integer tempH, Integer tempL) {
        Integer tempC = 0;
        Calendar cal = Calendar.getInstance();
    ...
    public static WeatherForestEntity cooeeForcastDataQuery(String s) {
    ...
    public static void CooeeFormatTemprature(WeatherForestEntity dataEntity,
            String temp) {
        String[] s = temp.split("\\,");
        String high = s[1];
        String low = s[0];
