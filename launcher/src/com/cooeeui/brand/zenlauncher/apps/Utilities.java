...
import com.cooeeui.brand.zenlauncher.LauncherApplication;
...
final public class Utilities {
    ...
    static byte[] ARGB8888ToRGBA8888(int[] src) {
        int length = src.length * 4;
        byte[] dst = new byte[length];
        for (int i = 0, j = 0; i < src.length; i++, j += 4) {
            dst[j] = (byte) ((src[i] >> 16) & 0xFF);
            dst[j + 1] = (byte) ((src[i] >> 8) & 0xFF);
            dst[j + 2] = (byte) (src[i] & 0xFF);
            dst[j + 3] = (byte) ((src[i] >> 24) & 0xFF);
        }
        return dst;
    }
    ...
    private static void initStatics(Context context) {
        ...
        sIconWidth = sIconHeight = 96;// (int) resources
    ...
    public static void startActivityForResultSafely(Activity activity,
